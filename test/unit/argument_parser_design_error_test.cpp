...
TEST(design_error, app_name_validation)
{
    ...
    EXPECT_NO_THROW((sharg::argument_parser{"test_parser", 1, argv}));
    EXPECT_NO_THROW((sharg::argument_parser{"test-parser1234_foo", 1, argv}));

    EXPECT_THROW((sharg::argument_parser{"test parser", 1, argv}),       sharg::design_error);
    EXPECT_THROW((sharg::argument_parser{"test;", 1, argv}),             sharg::design_error);
    EXPECT_THROW((sharg::argument_parser{";", 1, argv}),                 sharg::design_error);
    EXPECT_THROW((sharg::argument_parser{"test;bad script:D", 1, argv}), sharg::design_error);
}
...
TEST(parse_test, design_error)
{
    ...
    // short option
    const char * argv[] = {"./argument_parser_test"};
    sharg::argument_parser parser{"test_parser", 1, argv};
    ...
    // long option
    sharg::argument_parser parser2{"test_parser", 1, argv};
    ...
    // empty identifier
    sharg::argument_parser parser3{"test_parser", 1, argv};
...
TEST(parse_test, parse_called_twice)
{
    ...
    sharg::argument_parser parser{"test_parser", 5, argv};
...
TEST(parse_test, subcommand_argument_parser_error)
{
    ...
    // subcommand parsing was not enabled on construction but get_sub_parser() is called
    {
        ...
        sharg::argument_parser top_level_parser{"top_level", 2, argv, sharg::update_notifications::off};
    ...
    {
        const char * argv[]{"./top_level", "-f"};
        EXPECT_THROW((sharg::argument_parser{"top_level",
                                              2,
                                              argv,
    ...
    // no positional/options are allowed
    {
        ...
        sharg::argument_parser top_level_parser{"top_level", 2, argv, sharg::update_notifications::off, {"foo"}};
